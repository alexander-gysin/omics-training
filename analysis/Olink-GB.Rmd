...
```{r PCA_Plot_by_group, class.source = "fold-hide"}
...
```{r top_10_proteins, class.source = "fold-hide"}
...
```{r loading_plot, class.source = "fold-hide"}
...
### Volcano plot
```{r volcano_plotting, class.source = "fold-hide"}
# --- 2. THE FINAL PLOT ---
...
```
### MA Plot
```{r MA_plot, class.source = "fold-hide"}
...
## Protein Protein Interaction
```{r map_Gene_Symbols_to_String, class.source="hide-fold"}
...
<img src="assets/ppi_network.png" width="100%" alt="STRING Protein-Protein Interaction Network">
...
```{r Functional_Enrichment, class.source="fold-hide"}
...
```{r Visualization_GSEA, fig.width=8, fig.height=8}
...
```{r ridgeplotGSEA, fig.width=8, fig.height=13}
