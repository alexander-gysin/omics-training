...
gsea_sig <- gsea_results_df %>%
  ...
  arrange(desc(abs(NES))) # Order by the strongest effect size

# 3. Clean the GO Term names (MSigDB puts 'GOBP_' in front of everything)
gsea_sig$Description <- gsub("GOBP_", "", gsea_sig$Description)
gsea_sig$Description <- gsub("_", " ", gsea_sig$Description)

# Check how many significant terms remain
print(paste("Significant GO terms found:", nrow(gsea_sig)))
```
